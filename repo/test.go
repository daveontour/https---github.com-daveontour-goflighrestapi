...
func PerfTestInit(nf int, minProperties int) {
	...
	min, max := SendFlightMessages(nf, true)
	(*rep).CurrentLowerLimit = min
	(*rep).CurrentUpperLimit = max

	rep.MinimumProperties(minProperties)
	fmt.Println("Flights loaded. Ready to process requests via HTTP RestAPI")

}
func SendUpdateMessages(nf int) (time.Time, time.Time) {
	return SendFlightMessages(nf, false)
}
...
func SendFlightMessages(nf int, append bool) (minDate, maxDate time.Time) {
	...
	for i := 0; i < nf; i = i + 2 {
		...
		if globals.DemoMode {
			...
			UpdateFlightEntry(arrmsg, append)
			fmt.Printf("D")
			if (i+2)%100 == 0 {
				fmt.Printf("[%d]\n", i+2)
			}

			UpdateFlightEntry(depmsg, append)
			...
			if testConfig.TestConfig.Repository.ListenerType == "RMQ" {
				publishtopic(arrmsg)
			} else {
				writeToMQ(arrmsg, testConfig.TestConfig.Repository.NotificationListenerQueue)
			}
			fmt.Printf("D")
			if (i+1)%100 == 0 {
				fmt.Printf("[%d]\n", i+1)
...
func writeToMQ(message string, queue string) {
	opts := []msmq.QueueInfoOption{
		msmq.WithPathName(queue),
	}
	...
	writequeue, err2 := queueInfo.Open(msmq.Send, msmq.DenyNone)
	if err2 != nil {
		globals.Logger.Error(err2)
		return
	}
	...
	err = msg.SetBody(message)
	if err != nil {
		log.Fatal(err)
	}
	err = msg.Send(writequeue, msmq.SendWithTransaction(msmq.NoTransaction))
	if err != nil {
		log.Fatal(err)
	}
	err = writequeue.Close()
	if err != nil {
		log.Fatal(err)
	}
}
...
func publishtopic(message string) {
	conn, err := amqp.Dial(testConfig.TestConfig.Repository.RabbitMQConnectionString)
